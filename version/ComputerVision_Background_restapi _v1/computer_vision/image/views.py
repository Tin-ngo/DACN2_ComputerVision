...
def remove_background(request):
    ...
    # else:  # đây là nếu nhận được request là một chuỗi chứa path ảnh (nhưng mà sai vì lấy path cục bộ, máy khác thì không dùng được)
...
def change_background(request):
    ...
    if 'img_subject' in request.FILES:
        ...
        uploaded_image = str(request.data.get('path_img_subject'))
        input = Image.open(uploaded_image)
        file_name = os.path.basename(uploaded_image)
    ...
    if 'img_bg' in request.FILES:
        ...
        uploaded_image_bg =  str(request.data.get('path_img_bg'))
        input_bg = Image.open(uploaded_image_bg)
    ...
    return Response({"path_img_input_subject": "http://127.0.0.1:8000/static/image/preProcessing/" + file_name,\
                    "path_img_input_bg": "http://127.0.0.1:8000/static/image/changeBG/imageBG/" + file_name,\
